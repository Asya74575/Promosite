#!/bin/sh
# AWRIS demo on macOS: double-click (or run `sh start-demo.command`) to serve this folder and open the site.
# Opening index.html directly does not work: browsers block the site's scripts and 3D models on file://.
cd "$(dirname "$0")" || exit 1
PORT=5173
if command -v python3 >/dev/null 2>&1; then
  echo "AWRIS demo is running at http://localhost:$PORT/ (close this window to stop)"
